...
def main():
    ...
    print(network_training_output_dir, model, task_name, trainer + "__" +
                                          args.plans_identifier)
...
def predict_for_api(input_folder, output_folder, folds, tta, task_name):
    model = "3d_fullres"
    trainer = "nnUNetTrainerV2" #default_cascade_trainer
    plans_identifier = default_plans_identifier
    print(network_training_output_dir, model, task_name, trainer + "__" + plans_identifier)
    model_folder_name = join(network_training_output_dir, model, task_name, trainer + "__" + plans_identifier)
    print("using model stored in ", model_folder_name)
    assert isdir(model_folder_name), "model output folder not found. Expected: %s" % model_folder_name

    predict_from_folder(model_folder_name, input_folder, output_folder, folds, False, 16,
                        2, None, 0, 1, tta,
                        overwrite_existing=False, mode="normal", overwrite_all_in_gpu=None,
                        mixed_precision=True,
                        step_size=0.5, checkpoint_name="model_final_checkpoint", disable_postprocessing=False)
    return ""
