...
Причина — недетерминизм самой сегментации: ms-seg работает с TTA
(test-time augmentation) + неточность GPU-вычислений → маска каждый раз
чуть иная → другое число связных компонент. Это уровень модели, не
алгоритма счёта. Варианты: детерминированный инференс (фиксированный
seed, отключение TTA — ценой качества), или усреднение/ансамбль с
порогом. Связано с KI-018 (cascade/ensemble). Мин-фильтр размера очага
(см. ниже) частично снижает разброс за счёт удаления шумовых вокселей,
но корень — в модели.
