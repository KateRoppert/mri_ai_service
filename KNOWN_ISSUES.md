...
### KI-029 — Hardcoded `.dcm` extension по всему скрипту ✅ ЗАКРЫТО
**Приоритет:** был низкий, стал высоким при реальном столкновении
**Closed in:** commit `d2101ac` (`main`, 22.07.2026)

Подтвердилось на реальных данных: 3 из 19 пациентов `data/dropbox_33`
(KA117/KA119/KA120) хранят DICOM-файлы без расширения (`23831328` и т.п.).
Без фикса эти пациенты молча выпадали из прогона.

**Fix:** `is_dicom_file()` / `find_dicom_files()` — детекция через
Part-10 DICM-магию (128-байтный preamble + `b'DICM'` на смещении 128).
Файлы `*.dcm` идут быстрым путём без лишнего I/O (без изменений для
датасетов, которые уже используют это расширение). Применено во всех
7 местах обнаружения источника в `scripts/01_reorganize_folders.py`.
...
### KI-032 — Логи воркеров parallel-режима не доходят до файла лога ⚠️ УТОЧНЕНО
**Приоритет:** средний (DX/debuggability — подтверждено, что мешает расследованию)
**Branch:** `feat/prod-readiness`

Хуже, чем «перемешиваются»: `process_single_patient` создаёт **свой**
`logging.StreamHandler()` на процесс (`01_reorganize_folders.py`, метод
`process_single_patient`) — этот handler не подключён к файловому
handler'у главного процесса (`--log_file`). В файл лога попадает только
агрегированная строка `Patient X failed: <reason>` (её логирует
родительский процесс из `run_parallel`), а весь детальный контекст
(`logger.warning`/`.debug` внутри `_process_one_patient_core`, например
"Could not parse date from ...") виден только если процесс запущен вне
докера с прямым доступом к stdout воркера.

Реально мешало при расследовании dropbox_33 (KI кейс с датами): точная
причина сбоя была невидима в `stage_01_reorganize.log`, пришлось
воспроизводить `_process_one_patient_core` напрямую в Python, чтобы
увидеть настоящие warning'и. Решение: `logging.handlers.QueueHandler` +
`QueueListener` в основном процессе (пишет в общий файловый handler),
или per-process логфайлы с последующей склейкой.
...
### KI-047 — MosMed: полностью анонимизированные DICOM-теги ломают и дату, и модальность
**Приоритет:** средний (блокирует конкретный датасет, не общая архитектура)
**Branch:** не определена

`data/MosMed` (`EXCELMRI_..._anon`) падает не так, как dropbox_33 (см.
KI-029/date-fallback, закрыто commit `d2101ac`) — здесь глубже: у файлов
пустые `StudyDate`, `SeriesDate`, `AcquisitionDate`, `ContentDate`,
`PatientID`, `SeriesDescription`, `ProtocolName` — анонимизация стёрла
вообще весь текст, остались только числовые/geometric теги и UID'ы.
Структура папок — `patient/StudyInstanceUID/SeriesInstanceUID/*.dcm`.

И дата, и модальность в `01_reorganize_folders.py` зависят от текстовых
тегов (StudyDate для даты, ProtocolName/SeriesDescription для модальности
через keyword-паттерны) — оба сигнала отсутствуют одновременно. DICOM-
tag-фолбэк для даты (introduced в `d2101ac`) не помогает: тег есть, но
пустой, а не отсутствует.

Подтверждено: этот датасет падал так же 8 июля (`mosmed-8_07_1903`,
`mosmed-8_07_1907`), до расследования dropbox_33 — не регрессия
сегодняшнего фикса.

Возможное направление (не проработано): группировать сессии по
`StudyInstanceUID` вместо даты (UID всегда присутствует и уникален —
не требует парсинга даты вообще); для модальности — классификация по
техническим DICOM-параметрам (dimensions, spacing, `ScanningSequence`,
`SequenceVariant`) вместо текстовых keyword'ов, раз текста нет.

### KI-048 — Немецкий маркер контраста "KM" (Kontrastmittel) не распознавался ✅ ЗАКРЫТО
**Приоритет:** был низкий, повышен — реально терял данные пациента
**Closed in:** commit `f0f3aac` (`main`, 22.07.2026)

Уточнение по факту (изначальная запись недооценивала impact): контрастная
серия **не просто получала неверную метку — она полностью терялась**.
У `data/dropbox_33/KA53` были две серии: обычная T1 и T1-с-контрастом
(названа `"T1W_FFE 5mm tra KM"`). Обе классифицировались как `t1`
(контраст не распознан) → дедупликация выбирала одну по числу срезов →
контрастная серия отбрасывалась целиком, `t1c` отсутствовала у пациента
вообще, хотя обязательна для glioblastoma.

**Fix:** `_has_km_marker()` — зеркально `_has_ce_marker()` (word-boundary
regex, не substring), подключено во все 3 места, где уже проверялся `'ce'`.
KA53 теперь получает полный набор `t1/t1c/t2/t2fl`.

### KI-049 — Skull stripping (BET): три смежных бага при сбое
**Приоритет:** средний (реальный кейс молча портил результат конкретному пациенту)
**Branch:** не определена

Обнаружено при разборе сбоя BET на втором ноуте (пациент KA117, второй
этап препроцессинга): картинка оставалась с черепом, при этом пайплайн
рапортовал успех. Корневая причина — путь вывода содержал пробел
(`.../Рабочий стол/...`, русская локаль → пробел в имени папки Desktop),
а FSL печально известен тем, что многие его тулы — обёрточные
tcsh/perl-скрипты, не всегда корректно экранирующие пути с пробелами
внутри себя, даже когда путь передан снаружи одним аргументом. Подтверждено
экспериментально: тот же файл, скопированный в путь без пробела/кириллицы,
BET проходит (`EXIT_CODE:0`); на исходном пути падает (`Error: input image
... not valid`) — на обеих машинах код идентичен, различие чисто в том,
куда указывает `root_output_dir`.

Пока разбирались, всплыли ещё два самостоятельных бага в
`scripts/preprocessing_steps/skull_stripping.py`, из-за которых причина
сбоя не была видна в логе вообще:

1. **`run_bet()` теряет реальный текст ошибки.** Строка ~232:
   `raise RuntimeError(f"BET failed with return code {result.returncode}: {result.stderr}")`
   — берёт только `result.stderr`, а BET печатает своё "Error: input image
   ... not valid" в stdout. В логе сообщение об ошибке было пустым
   (`BET failed with return code 1: `), реальная причина не диагностировалась
   без ручного запуска BET напрямую.
2. **Сбой BET не проваливает шаг.** После исключения из `run_bet()` шаг
   skull stripping всё равно логируется как `✓ Skull stripping completed
   in 0.1s`, и препроцессинг для пациента репортует 100% success rate —
   притом что brain mask не создана и итоговая картинка осталась с черепом.
   Это тихий провал (silent failure): врач не узнаёт, что сегментация
   пойдёт на неправильных данных, пока не увидит результат глазами.

Возможное направление (не проработано): (а) копировать входной файл во
временный путь без пробелов/non-ASCII перед вызовом BET, чтобы не зависеть
от того, куда пользователь указал `root_output_dir`; (б) собирать
`result.stdout + result.stderr` в сообщение об ошибке, как уже сделано в
`check_fsl_installed()` (там это уже правильно, см. `output = result.stdout
+ result.stderr`); (в) пробрасывать исключение из `run_bet()` дальше как
фатальную ошибку шага, а не глотать его.

### KI-050 — `register_ai_mask` замораживает путь к маске навсегда, Slicer молча остаётся без маски
**Приоритет:** высокий (ломает основной сценарий ручного редактирования маски)
**Branch:** не определена

Обнаружено 2026-08-13 при ручной проверке: открытие пациента в 3D Slicer
для ручного редактирования маски (`/api/slicer/open/{run_id}`) не
подгружало маску, хотя раньше работало. Корневая причина — не регрессия
от конкретного коммита, а давняя недоработка в `register_ai_mask()`
(`backend/mask_service.py:16-56`).

`register_ai_mask` создаёт запись `MaskVersion(entity_id, version=1)`
только один раз на `entity_id`. Если запись уже есть — функция молча
возвращает её как есть, не обновляя `file_path`:
...
Каппа при повторной загрузке одних и тех же исходных данных всегда
резолвит их в одну и ту же постоянную сущность (entity_id) — подтверждено
логами (`Reconciled entity after ambiguous upload`). Значит, при первой
успешной регистрации маски путь к файлу замораживается навсегда: сколько
бы раз пациента ни переобрабатывали заново (новый run, новая папка
вывода, новая маска на диске), `mask_versions` продолжает указывать на
файл из самого первого прогона.

`GET /api/slicer/open/{run_id}` (`backend/app.py:1861`) корректно
фильтрует несуществующие пути (`if not fp or not Path(fp).exists():
continue`) — так что как только та самая первая папка вывода когда-нибудь
удаляется (обычное дело при повторном тестировании одних и тех же демо-
датасетов), маска молча исчезает из Slicer, хотя в текущем прогоне лежит
рабочая свежая маска.

Подтверждено на реальных данных: `mask_versions` id=273 (`entity_id
06e9db72-76c9-4d22-bc46-d381090470f9`, пациент BO-1/sub-092) содержит
`file_path`, созданный 2026-08-05 и указывающий на
`demo_workspace/input/BO/segmentation/...` — папки уже не существует.
Прогон `13_08_1806` от 2026-08-13 сгенерировал новую маску в новой папке,
но `mask_versions` про неё не узнал.

Соседняя `register_expert_mask` (та же секция файла) устроена правильно —
всегда пишет новую версию, а не молчит поверх старой. Предлагаемый фикс:
при существующей version=1 обновлять её `file_path`/`file_name` на
актуальные, а не возвращать без изменений. Небольшое, локальное изменение
одной функции — не затрагивает `feat/incomplete-patients-frontend` и
может быть сделано отдельной веткой.

### KI-051 — Requeue: автоматическая очередь вместо ручного повтора после завершения текущего прогона
**Приоритет:** средний (удобство, не блокер)
**Branch:** не определена

Сейчас `POST /api/pipeline-runs/{run_id}/requeue` (`backend/app.py`) отвечает
409 и фронтенд блокирует кнопку «Запустить обработку», пока на этом
`output_path` уже выполняется другой прогон (`get_active_run_by_output_path`,
`backend/database.py`) — намеренная защита от двух orchestrator-процессов,
конкурентно пишущих в один `bids_organized/dataset_mapping.json` (см.
финальное ревью `feat/incomplete-patients-frontend`, Critical-находка).

Высказанное пожелание (2026-08-14): дать врачу нажать «Запустить обработку»
сразу, не дожидаясь завершения текущего прогона, а не блокировать кнопку —
запрос должен встать в очередь и запуститься автоматически, как только
текущий прогон на этом пути завершится.

Отложено сознательно: нужен способ хранить факт «на этот output_path
запрошен ещё один прогон» (например, булево поле на активном `PipelineRun`
или отдельная небольшая таблица), и место, которое это проверяет по
завершении прогона и сама вызывает то же самое, что делает
`requeue_pipeline_run` — естественный кандидат: там же, где
`pipeline_monitor.py` уже реагирует на завершение прогона и запускает Kappa
upload (`_monitor_loop`, `if run.status in ["completed","failed"]`). Не
проработано: что если за время ожидания в очереди врач передумал или
разметил что-то ещё — очередь должна брать актуальное состояние
`dataset_mapping.json` на момент фактического запуска, а не на момент
постановки в очередь (уже верно для обычного requeue, т.к. `skip_existing`
читает состояние на диске в момент старта, а не в момент клика).

### KI-052 — Таймаут пайплайна (2ч) не масштабируется на полный датасет, а на срабатывании убивает только orchestrator, не дочерние этапы
**Приоритет:** критический (реальные результаты тихо теряются из веб-интерфейса на полном прогоне)
**Branch:** не определена

Обнаружено 2026-08-17 при разборе прогона на полном `data/clinical_dicom/BO`
(175 пациентов, `run_id=e4ad2ec9...`, 2026-08-14). В БД (`pipeline_runs`)
запуск помечен `status=failed`, `error_message="Превышено время ожидания
выполнения"`, `completed_at=14:27:51` (ровно 2 часа после старта в 12:27:51).
Но по факту:

- Собственный лог этапа 5 (`stage_05_preprocessing.log`) говорит, что этап
  **успешно завершился в 13:37:13** — за 50 минут ДО того, как бэкенд
  объявил весь прогон проваленным по таймауту.
- Этап 6 (`stage_06_segmentation.log`) продолжал писать реальные логи
  сегментации **до 15:16:10** — почти 2 часа ПОСЛЕ того, как БД уже
  показывала врачу «failed», с корректным summary (159/160 сессий, отчёт
  `segmentation_incomplete_data.json` сохранён).

Корень: `backend/config.py:44` — `pipeline_timeout_seconds: int = 7200`
(2 часа), единая жёсткая граница на ВЕСЬ многоэтапный прогон целиком,
независимо от размера датасета. `backend/app.py:201`:
`process.communicate(timeout=settings.pipeline_timeout_seconds)` — таймаут
относится к процессу `orchestrator.py`. При срабатывании (`except
subprocess.TimeoutExpired`, `backend/app.py:251-262`) вызывается
`process.kill()` (строка 254) — но это убивает только САМ `orchestrator.py`,
а не его дочерний процесс текущего этапа (`05_preprocessing.py` /
`06_segmentation.py` и т.д.), запущенный через `subprocess.Popen`/`.run()`
внутри orchestrator'а. `Popen.kill()` не рекурсивен — дочерний процесс
дочернего процесса становится сиротой и продолжает работать независимо,
пока не завершится сам (или не рухнет), полностью невидимо для БД/веб-
интерфейса.

Итог для врача: экран показывает «Ошибка выполнения» через 2 часа, врач
уходит разбираться в чём дело или перезапускает — а тем временем реальные,
корректные результаты сегментации для 159 из 160 пациентов продолжают
писаться на диск ещё почти 2 часа и никогда не попадают в UI (запись в БД
уже `failed`, никто их не подхватывает).

Два независимых фикса нужны вместе:
1. **Таймаут должен масштабироваться с размером датасета** (или считаться
   per-stage, а не на весь прогон целиком) — 161-175 пациентов на полном
   датасете обгоняют константу, рассчитанную на демо-прогоны на 5-20.
2. **При таймауте убивать всё дерево процессов**, не только прямого
   потомка — например `os.killpg` с процесс-группой (запускать orchestrator
   через `start_new_session=True`/`preexec_fn=os.setsid`, убивать через
   `os.killpg(os.getpgid(process.pid), signal.SIGKILL)`), либо явно
   отслеживать и убивать PID текущего этапа.

Вероятно, это и есть причина жалобы «если хотя бы одна сессия не была
сегментирована, пайплайн не доходит до конца» — не сбой самой сегментации
на конкретном пациенте (see KI-053, сегментация корректно пропускает
неполные модальности и не виснет), а то, что ПОЛНЫЙ прогон на реальном
датасете просто не укладывается в захардкоженные 2 часа, и врач узнаёт об
этом как о полном провале, хотя почти всё уже готово.

Примечание: group-kill процесс-группы, введённый для таймаута, теперь также лежит в основе `POST /api/pipeline-runs/{run_id}/stop`.

### KI-053 — Reorientation падает с `NoneType` на affine с вырожденной ориентацией, молча теряя обязательную модальность
**Приоритет:** высокий (реальный кейс: пациент 214 в BO навсегда останется неполным без ручного вмешательства)
**Branch:** не определена

Обнаружено 2026-08-17, воспроизведено дважды (прогоны 2026-08-14 и
2026-08-17) на пациенте BO-214/sub-214: `t1c` стабильно падает на этапе
reorientation:

```
ERROR - Error reorienting sub-214_ses-001_t1c.nii.gz: sequence item 2: expected str instance, NoneType found
```

Корень — `scripts/preprocessing_steps/reorient.py:38-39`:
```python
original_orientation = nib.aff2axcodes(img.affine)
original_orientation_str = ''.join(original_orientation)
```
`nib.aff2axcodes` возвращает `None` вместо буквы оси, если affine-матрица
файла вырождена/нестандартна (например, из-за особенностей DICOM→NIfTI
конвертации на этапе 03 для конкретной серии) — `''.join(...)` на кортеже
с `None` кидает ровно это исключение. `t1`, `t2`, `t2fl` того же пациента
реориентируются нормально — проблема специфична для конкретного NIfTI-файла
`t1c`, не для пациента в целом.

Исключение перехватывается (`except Exception` в `reorient_to_standard`,
строки 85-90) и возвращает `{"success": False, ...}` — сам процесс не
падает. Но дальше по пайплайну это тихо превращается в потерю модальности:
`bias_correction` логирует `WARNING - No file found for modality t1c` и
просто пропускает её, `t1`/`t2`/`t2fl` доходят до конца этапа 5 нормально,
и пациент помечается `✓ Completed sub-214/ses-001` — **несмотря на то, что
обязательная для glioblastoma модальность t1c отсутствует**. Только на
этапе 6 (`06_segmentation.py`) это наконец обнаруживается
(`⚠️ Skipping sub-214_ses-001: Missing modalities {'t1c'}`) и пациент
пропускается — то есть ошибка молча кочует через два этапа пайплайна,
прежде чем где-либо проявиться.

Предлагаемое направление: (а) не полагаться на `nib.aff2axcodes` вслепую —
проверять `None` в возвращённом кортеже до `''.join()` и явно логировать
«вырожденная/нестандартная ориентация affine» вместо голого `TypeError`-
подобного сообщения; (б) разобраться, почему именно для этого файла affine
оказался вырожденным — стоит проверить исходный DICOM `t1c`-серии пациента
214 и/или её конвертацию на этапе 03 (это может быть тем же классом
проблемы, что и уже закрытые баги DICOM→NIfTI конвертации, KI, связанные с
Stage 03); (в) главное — эта потеря модальности должна попадать в
`excluded_series`/incomplete-patients очередь ещё на этапе 5, а не
обнаруживаться молча только на этапе 6. Сейчас пайплайн для несегментиро-
ванного из-за этого пациента просто помечает его "неполным" в отчёте
сегментации, но врач в веб-интерфейсе (`IncompletePatients.jsx`) видит
только очередь review с этапа 1 — сессия, ставшая неполной на этапе 5,
туда не попадает вообще (см. KI-054 ниже, тот же корень).

### KI-054 — Отчёт о потерянных пациентах существует на диске на каждом этапе, но никогда не показывается врачу
**Приоритет:** высокий (напрямую запрошено врачом; данные уже есть, нужен только UI)
**Branch:** не определена

Обнаружено 2026-08-17. Каждый этап пайплайна (01, 03, 04, 05, 06 — 07 и 08
пока НЕТ, см. ниже) уже самостоятельно пишет `{stage}_incomplete_data.json`
в свою выходную директорию с точной статистикой:

```json
{
  "timestamp": "...", "stage": "06_segmentation",
  "statistics": {"total_patients": 170, "complete_patients": 169,
                  "incomplete_patients": 1, "success_rate_percent": 99.41},
  "incomplete_data": [
    {"patient_id": "214", "incomplete_sessions": [
      {"session_id": "001", "input_modalities": ["t1","t2","t2fl"],
       "output_data": false, "reason": "patient_missing_in_output"}
    ]}
  ]
}
```

Файлы подтверждены на реальном прогоне BO:
`bids_organized/incomplete_data/01_reorganize_folders_incomplete_data.json`,
`nifti/incomplete_data/03_convert_to_nifti_incomplete_data.json`,
`preprocessed/incomplete_data/preprocessing_incomplete_data.json`,
`quality_reports/incomplete_data/04_assess_quality_incomplete_data.json`,
`segmentation/incomplete_data/segmentation_incomplete_data.json`.
Ни один из них сейчас нигде не читается бэкендом и не отдаётся в UI.

Значит задача №3 (запрошена явно: «понятный отчёт о том, сколько пациентов
было на входе каждого этапа и кто потерялся на выходе, только инфо о
потерянных, не вся таблица») в основном сводится к **чтению уже
существующих файлов и агрегации в один отчёт**, а не к постройке нового
трекинга с нуля — это заметно дешевле, чем казалось на этапе
брейншторминга «сквозного аудита» (см. `docs/superpowers/specs/
2026-08-13-review-queue-audit-trail-design.md`, где это было явно вынесено
за скобки как «материально более крупная фича»).

Что нужно для реализации: (а) новый backend-метод, читающий все
`{stage}_incomplete_data.json` файлы для `output_path` запуска и
объединяющий их в один список «пациент X потерян на этапе Y по причине Z»;
(б) закрыть пробел для этапов 07/08 — они сейчас вообще не пишут такой
отчёт (`grep` по `07_inverse_transform.py`/`08_anatomical_analysis.py` не
находит `incomplete_data.json`), значит патчить и их тоже, по тому же
формату; (в) новый эндпоинт + место в UI — вероятно, отдельная вкладка/
секция в `ProgressMonitor`/`PipelineHistory`, независимая от очереди
review из `IncompletePatients.jsx` (та привязана к этапу 1, а тут нужен
взгляд по всем этапам сразу).

### KI-055 — Объединённые (merged) сессии должны переставать быть permanent-записью после повторного прохода этапа 1
**Приоритет:** средний (уточнение уже реализованной фичи ручного объединения сессий, `feat/manual-session-merge`)
**Branch:** не определена

При проектировании ручного объединения сессий (`docs/superpowers/specs/
2026-08-14-manual-session-merge-design.md`) статус `merged` был сделан
постоянной audit-trail записью по аналогии с `discarded` — сессия-донор
навсегда остаётся в очереди review с тегом «Объединена → ses-XXX»
(`backend/pipeline_manager.py::get_incomplete_patients`, клауза
`status == 'merged'`).

Уточнение от врача (2026-08-17): после того, как врач воспользовался
перенесёнными сериями (назначил их через обычный `relabel_series`) и
запустил повторный прогон (`requeue`), который заново прошёл этап 1 —
донорская `merged`-сессия должна ПЕРЕСТАТЬ показываться в очереди, а не
оставаться там навсегда. В отличие от `discarded` (осознанный отказ от
данных, есть смысл помнить вечно), `merged` после реального повторного
прогона теряет практическую ценность как «напоминание» — исходная сессия
и так помечена терминальным статусом, и постоянно маячить в списке уже
незачем.

Не проработано: как отличить «прошёл повторный прогон этапа 1 после
объединения» от «просто снова открыли отчёт» — вероятно, нужен маркер
вроде `merged_at_run_id` (id запуска, при котором произошло объединение),
сравниваемый с текущим `run_id` при чтении `get_incomplete_patients` —
если текущий запрос идёт из НОВОГО run_id (то есть после реального
requeue), `merged`-сессия из предыдущего run_id больше не показывается.
Нужно решить, требует ли это привязки к `parent_run_id`
(`feat/requeue-progress-linking`) или работает независимо через сам факт
смены run_id для того же `output_path`.


### KI-056 — Slicer не открывает маску из вкладки основного запуска на втором рабочем месте
**Приоритет:** средний (воспроизводится только на второй машине; на основной работает)
**Branch:** не определена

После разворачивания версии `8bb4f0a` на втором ноутбуке (полный перенос:
`git pull` + пересборка образов + копирование весов) кнопка редактирования
в 3D Slicer из вкладки «Запуск обработки» не открывает маску. Тот же
сценарий на основной машине отрабатывает штатно.

Что уже проверено и НЕ является причиной:

- Различия в незакоммиченных `configs/kappa_datasets.yaml` и
  `pipeline_config.yaml` между машинами — это осознанно, на второй машине
  свои параметры и свой Kappa-датасет (268).
- Пустая БД / несработавшая выгрузка в Kappa: `mask_versions` и
  `patient_registry` заполнены, `kappa_entity_id` проставлены, файлы
  сегментации лежат по путям вида
  `/home/ubuntu/brain_lesion_results/KA1XX/segmentation/...`.
- Kappa в целом доступна: в логах успешные запросы и скачивание файлов.

Замеченная нестыковка, с которой стоит начать: в БД и в логах Slicer
фигурируют sub-002 и sub-018..sub-022, тогда как прогонялся sub-001 —
то есть записи по свежему запуску в выборке не обнаружились. Также в
логах присутствует только вызов `open-from-kappa` (вкладка валидации),
клинического `POST /api/slicer/open/{run_id}` в собранном фрагменте лога
не видно.

Диагностика не завершена, нужны с второй машины:

1. `SELECT ... FROM patient_registry WHERE bids_id LIKE 'sub-001%'` и
   соответствующие строки `mask_versions` — есть ли вообще свежий запуск.
2. Строка `Slicer open: patient=..., default_mask=..., ai=N, expert=N`
   из лога `web` сразу после нажатия кнопки — нашёл ли бэкенд маску.
3. Существование файла по `file_path` из БД (на второй машине свой
   `output_path`, не `demo_workspace`).
4. Поведение глазами: Slicer не открывается вовсе / открывается со
   снимками, но без маски / показывает ошибку.

Гипотезы, отсеиваемые пунктами выше: маска не зарегистрирована для
конкретно этого запуска; путь в БД указывает на несуществующий файл;
фронт не отправляет запрос, потому что не определился `runId`/`entityId`
(тогда в логе не будет вызова `/api/slicer/open/`).

Отложено 2026-08-21 по решению владельца — приоритет отдан удалённому
прогону пайплайна.


### KI-057 — Жёсткий `dataset_id` в конфиге ломает работу под чужой учётной записью
**Приоритет:** средний (проявится при первом же стороннем пользователе)
**Branch:** не определена

`KappaUploader` берёт токен, `user_id` и `user_type_id` из сессии вошедшего
пользователя (`backend/pipeline_monitor.py::_create_uploader`) — это верно.
Но `_resolve_dataset_id()` первым делом читает `configs/kappa_datasets.yaml`
и, если запись есть, возвращает её без проверки доступности:

```python
dataset_id = get_dataset_id(self.lesion_type, self.preprocessing_id)
if dataset_id is not None:
    return dataset_id          # права на датасет не проверяются
```

В результате получается комбинация «токен пользователя A + датасет
пользователя B». Пока машина и пользователь одни, разницы нет. Как только
появляется второй пользователь (сценарий удалённого запуска на barguzin,
`docker-compose.barguzin.yml`), запись в конфиге начинает означать «пиши в
чужой датасет», и выгрузка падает по правам — причём уже ПОСЛЕ того, как
весь пайплайн отработал, то есть впустую тратится полный прогон.

Ветка автосоздания ниже по коду делает ровно то, что нужно (заводит датасет
под текущим пользователем и запоминает id через `set_dataset_id`), но
существующая запись в конфиге до неё не даёт дойти. То есть
`kappa_datasets.yaml` фактически работает как кэш «датасет уже заведён», а
не как настройка, и на многопользовательском сценарии это ломается.

Что стоит сделать:

1. Проверять доступность датасета до запуска пайплайна, а не при выгрузке —
   ошибка прав должна всплывать до того, как потрачены минуты GPU.
2. Сделать ключ маппинга пользовательским: сейчас это
   `lesion_type + preprocessing_id`, нужен ещё `user_id`. Тогда у каждого
   пользователя свой датасет, и конфиг перестаёт быть общей точкой отказа.

Временный обход для barguzin: удалить записи из `configs/kappa_datasets.yaml`
на той машине — тогда при первом запуске датасеты создадутся под учётной
записью того, кто вошёл.

Обнаружено 2026-09-02 при подготовке удалённого запуска для научного
руководителя.

---

### KI-058 — Загрузка атласа молча сохраняет HTML-страницу как `.nii.gz`

**Приоритет:** средний (пока не срабатывает — атлас лежит в кэше; выстрелит на чистой машине)
**Branch:** не определена

`download_sri24_atlas()` (`scripts/preprocessing_steps/registration.py:46`)
пишет тело HTTP-ответа в файл без единой проверки, что это NIfTI. Любой
ответ с кодом 200 — включая страницу ошибки NITRC — считается успешной
загрузкой и логируется как `Atlas downloaded successfully`. Блок `except`
удаляет файл только при исключении, а HTML-страница исключения не вызывает.

Вторая половина проблемы: имя файла берётся из `atlas.name`, а URL — из
`atlas.url`, и они между собой не связаны. При `name: "MNI152_FSL"` скачивание
идёт по зашитой ссылке на **SRI24** и сохраняется под именем MNI152. То есть
даже успешная загрузка дала бы не тот атлас, что указан в конфиге.

Обнаружено 2026-09-09: в репозитории нашёлся `scripts/data/templates/
MNI152_T1_1mm.nii.gz` — 39 КБ XHTML вместо атласа. Появился потому, что
`atlas.cache_dir: "data/templates"` — относительный путь, и при запуске с
рабочим каталогом `scripts/` он разрешился в `scripts/data/templates`.
Настоящий атлас в `data/templates/` не был найден, сработала ветка загрузки.
Файл удалён.

Что стоит сделать:

1. После загрузки открывать файл через `nibabel` и падать, если это не NIfTI.
2. Разрешать `cache_dir` относительно корня проекта, а не CWD.
3. Связать URL с `atlas.name` (таблица имя → ссылка) либо запретить загрузку
   для шаблонов, у которых ссылки нет, с внятной ошибкой.
