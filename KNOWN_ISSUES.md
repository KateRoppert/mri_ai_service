...
- Коэффициенты `k_stage` для этапов 05/06/07/08 ещё не измерены — измерить через
  `scripts/system_monitor.py` и занести сюда как таблицу. Это готовый датасет для
  обучения/калибровки политики планировщика.
