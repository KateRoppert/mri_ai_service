...
### KI-048 — Немецкий маркер контраста "KM" (Kontrastmittel) не распознаётся
**Приоритет:** низкий
**Branch:** не определена

`ModalityDetector`'s `contrast_keywords` список (`01_reorganize_folders.py`)
содержит только английские маркеры (`post`, `gad`, `contrast`, `c+`,
`enhanced`, `+c`). У `data/dropbox_33/KA53` серия называется
`"T1W_FFE 5mm tra KM"` — "KM" = Kontrastmittel (нем. «контрастное
вещество») — классифицируется как `t1`, а не `t1c`. Серия всё равно
копируется и не теряется, просто с неверной contrast-меткой.

Затрагивает любой немецкоязычный клинический источник. Кандидат fix:
добавить `'km'` в `contrast_keywords` (с word-boundary проверкой, как
уже сделано для `'ce'` через `_has_ce_marker`, — иначе будет ловить
случайные вхождения `km` внутри других слов).
