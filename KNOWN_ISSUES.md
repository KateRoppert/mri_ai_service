...
### KI-045 — Качество сегментации РС: гигантский блоб + шум фрагментами
**Приоритет:** высокий (клиническая достоверность)
**Branch:** `main` (выявлено при разборе оттока очагов, 17.07.2026)

При разборе лонгитюда sub-004 (P000067) обнаружено в label-масках:
- **один компонент ~24 см³** (24391 вокселей) — для очага РС нереально много,
  вероятно ложноположительная зона (спутанные перивентрикулярные очаги / CSF /
  артефакт), присутствует стабильно в разных сессиях;
- **~200 крошечных фрагментов** (медиана 2 вокселя = 0.002 см³) — большинство
  ниже клинического порога (5 мм³) и отфильтровывается в diff, но зашумляет
  подсчёт «очагов» в отчёте.

Нужно: QC сегментации — верхний порог на размер компонента (флаг «подозрительно
большой»), морфологическая чистка мелких фрагментов, ревизия числа «очагов» в
клиническом отчёте. Проверить, не деградирует ли ms-seg на данных SibBMS 0.35мм
(ресемпл в 1мм атлас) относительно данных, на которых обучался.

### KI-046 — Настоящая лонгитюдная ко-регистрация сессий
**Приоритет:** высокий (клиническая достоверность лонгитюда)
**Branch:** `main` (выявлено при разборе оттока очагов, 17.07.2026)

Каждая сессия регистрируется к атласу **независимо**. Для лонгитюда этого мало:
у sub-004 соседние сессии выровнены Dice 0.42–0.92 (норма 0.98 у sub-002/003),
а ses-006 — обрезанный по FOV скан (мозг только Z[67..154]) → очаги не
пересекаются → ложная «активность». Сейчас закрыто **guardrail'ом** (KI —
`coregistration_dice` + флаг `reliable=false`, коммит `935761d`): недостоверные
пары помечаются, а не показывают мусор. Но это защита, а не решение.

Нужно (стандарт лонгитюдных МС-пайплайнов): регистрировать сессии **друг к другу
или к субъект-специфичному шаблону** (halfway-space), а не независимо к общему
атласу; отбраковывать/помечать сессии с неполным покрытием мозга до сегментации.
Связано с исследованием skull-stripping (Этап 5.5) и Stage 05/07.
