...
USE_REFACTORED_VERSION = True  # Начинаем с False для безопасности
...
if USE_REFACTORED_VERSION:
    from dicom_to_bids.organizing.adapter import BidsOrganizerAdapter
    
    class BidsOrganizer(BidsOrganizerAdapter):
        def __init__(self, output_dir, action_type='copy', 
                    max_parallel_files=32, max_workers=None):
            super().__init__(output_dir, action_type, max_parallel_files, max_workers)
            # Автоматически создаём и устанавливаем детектор
            detector = EnhancedModalityDetector()  # Ваш существующий класс
            self.set_detector(detector)
            self.detector = detector  # Для совместимости
else:
