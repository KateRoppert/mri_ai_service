...
class MetadataExtractor:
    ...
    def anonymize_dicom(self, dcm: pydicom.Dataset) -> List[str]:
        """
        Remove all tags listed in tags_config from DICOM dataset (in-place).
        
        Args:
            dcm: pydicom Dataset to anonymize
            
        Returns:
            List of tag names that were removed
        """
        removed = []
        for group_name, tags in self.tags_config.items():
            for tag_name, tag_info in tags.items():
