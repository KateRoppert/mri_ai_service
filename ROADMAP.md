...
**Ветка:** `feat/skull-stripping-research-v2` (июньская `feat/skull-stripping-research` оставлена как архив)
**Зависит от:** Этапа 5
**Тип:** Research / optimization
**Статус:** ⏳ В работе. Плагин BET+HD-BET и GPU-пул уже на `main`. Бенчмарк и остальные инструменты — по плану `docs/superpowers/plans/2026-09-04-skull-stripping-research.md`. Не блокирует Этап 6.

Сравнительное исследование инструментов skull stripping в рамках пайплайна.
FSL BET был подобран эмпирически; HD-BET уже доступен как `method: hdbet` (литература, не наш бенчмарк). Для гетерогенных данных (глио + МС) выбор победителя ещё не измерен.

**Содержание:**

- **Уже в проде:** конфиг-driven dispatcher, BET, HD-BET 2.x, fallback, GPU-пул (`exclude_gpus`).
- **Дальше:** cascade + проверка маски; SynthStrip, BrainMaGe, MNI strict/loose, SAM/DeepBET (best-effort); бенчмарк на 4 датасетах; статья.
- **Вывод:** 2 production winners по evidence, не по цитате.
...
2. ⏳ **`feat/skull-stripping-research-v2`** (Этап 5.5)
   Плагин BET+HD-BET на `main`. Дальше — план 2026-09-04 (cascade, остальные
   инструменты, бенчмарк, статья). Не блокирует Этап 6.
