...
**Зависит от:** Этапа 5, входных данных от врачей
**Статус:** ⏳ Ожидание инфо от клиницистов
...
**Содержание (зависит от клинических требований):**

- **McDonald-критерии локализации:** классификация очагов на
  перивентрикулярные / юкстакортикальные / инфратенториальные / спинной мозг.
  Требует алгоритма расстояния до желудочков/коры — изменение Stage 08.
- **Детекция новых/растущих очагов** между сессиями: inter-session
  registration + mask diff. Наиболее ценная метрика для мониторинга терапии.
- **Гадолиний-накапливающие очаги** (если T1c входит в МС-протокол).
- Обновление лонгитюдной панели под новые метрики.

**Цель:** МС-отчёт, полностью соответствующий клиническому workflow
(мониторинг прогрессии, оценка ответа на терапию).
...
**Зависит от:** Этапов 4, 5 (5.5 и 5.6 желательны но не блокируют)

Подготовка к развёртыванию в реальной клинической среде.

**Содержание:**

- **Kappa dedup (KI-035):** content-aware update при повторном прогоне
  пациента — обновлять entity вместо skip, если входные данные полнее.
- Покрытие тестами (unit + integration для критических путей)
- Документация: README, Architecture overview, API docs, Deployment guide
...
- UX-фиксы из KI: KI-026, KI-036, KI-040, KI-041
...
**Очередь следующих веток (по порядку):**

1. 🎯 **`feat/skull-stripping-research`** (Этап 5.5)
   Сравнительное исследование инструментов skull stripping.
   Начать сейчас.

2. ⏳ **`feat/ms-clinical-metrics`** (Этап 5.6)
   МС-отчёт Вариант C: McDonald-локализация + детекция новых очагов.
   Ждём входные данные от врачей о клинических требованиях.

3. 📦 **`feat/prod-readiness`** (Этап 6)
   Релизная подготовка + KI-035 (Kappa dedup). Начинается после 5.5.

---

*Последнее обновление: планирование веток 5.5/5.6/6, июнь 2026.*
