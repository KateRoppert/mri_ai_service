...
def _series(tmp_path, name, modality, n_files=1, is_derived=False):
...
class TestSelectBestSeries:
    def test_picks_primary_over_derived_reconstruction(self, deduplicator, tmp_path):
        primary = _series(tmp_path, "CE_T1-TFE (3D brain)", "t1c")
        derived = _series(tmp_path, "MPR CE_T1-TFE 2.5mm (cor brain)", "t1c", is_derived=True)
        best = deduplicator._select_best_series([primary, derived], "t1c")
        assert best is primary

    def test_derived_reconstruction_used_as_last_resort_when_only_candidate(self, deduplicator, tmp_path):
        """The whole point of scoring instead of hard-excluding: a
        reconstruction is still usable data when nothing else exists for
        this modality slot."""
        only = _series(tmp_path, "MPR CE_T1-TFE 2.5mm (cor brain)", "t1c", is_derived=True)
